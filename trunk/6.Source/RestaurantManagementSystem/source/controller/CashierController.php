<?php
require_once '../dal/billingDAO.php';
class CashierController{
	
    /*
     * @author thanhtuan
     * @parram $date 
     * @parram $totalMoneyFrom
     * @parram $totalMoneyTo
     * return gui contain information billing
     */
	public function searchBilling($date,$totalMoneyFrom,$totalMoneyTo){
		
		$dao = new billingDAO() ;
		$array = $dao->searchBillDAO($date, $totalMoneyFrom,$totalMoneyTo);
       ...
       <table class='unpaid-bill-table'>
		...
		foreach($array as $value){
			...
			$MaPhieu = isset($value["MaPhieuDatCho"]) ? $value["SoLuong"] : "";
			if($TinhTrang == 0)
			{
			$data = $data."<tr>";
...
switch ($action) {
	case "search":
		$totalMoneyFrom = isset($_REQUEST["totalMoneyFrom"]) ? $_REQUEST["totalMoneyFrom"] : "";
		$totalMoneyTo= isset($_REQUEST["totalMoneyTo"]) ? $_REQUEST["totalMoneyTo"] : "";
		...
		try {
			...
			$SearchResult = $search->searchBilling($date_format,$totalMoneyFrom,$totalMoneyTo);
			echo $SearchResult;
		} catch (Exception $e) {
