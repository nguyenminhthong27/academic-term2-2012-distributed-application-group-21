...
{
    reportGrid = new dhtmlXGridObject("reportDiv");
    reportGrid.setImagePath("dhtmlx/imgs/");//path to images required by grid7\4
    reportGrid.setHeader(",MÃ MÓN ĂN, TỔNG DOANH THU (VND)");//set column names
...
function showDateTimeCriteria(radio){
    ...
    var tdShowArr = $('#reportingCriteriaDiv .dateInput:nth-child(' + index+ ') td').get();
    for(var i=0; i<tdShowArr.length; i++){
        tdShowArr[i].style.display = "table-cell";
    }
}

/*make report when report button clicked
 *load data for grid and refresh chart
 *@param null
 *@return null
 *@author hathao298@gmail.com
 **/
 ...
 *@param dateRange make report on days, months or years, or all
 *       date date of report
 *@return null
 *author hathao298@gmail.com
 **/
...
/*parse date to get return type
 *@param date datetime dateformat 05/01/2012 00:00 mm/dd/yyyy
 *@param returnType int 1-date
 *          2 - month
 *          3 - year
 *@return returnType
 *          month - 9/2012
